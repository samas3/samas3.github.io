...
(()=>{
    ...
    (()=>{
        ...
        // 版本
        self._i = ["rks查询工具", [1, 1, 0], 1655595109, 0];
        function o(e) {
            ...
            t && function(t) {
                ...
                n.onload = async function() {
                    ...
                    await async function(e) {
                        const t = await fetch(decodeBase64('aHR0cHM6Ly9sY2h6aDM0NzMuZ2l0aHViLmlvL3Jrcy1wcm9iZS9zb25nLmpzb24=')).then((e=>e.json()))
                        ...
                        o.unshift(o.filter((e=>100 == e.acc)).slice(0, 3));
                        const a = o.slice(0, 30).reduce(((e,t)=>e + 5 * t.rating), 0)
                          , s = ((a + .5 | 0) - a + .5) / 5
                          , l = o[2].rating
                          , c = o[29].rating + s
                        ...
                        d.innerHTML+='\n背景颜色说明: <span class="sb">继续推分</span> <span class="sg">需要收歌才能涨rks</span> <span class="sr">无法推分</span>'
                        o.forEach(((e,t)=>{
                            if(t > 29 && document.getElementById("b19").checked) return;
                            const n = document.createElementNS("http://www.w3.org/2000/svg", "svg");
                            n.classList.add("svg"),
                            n.style.cssText += `;width:calc(${100 / r}% - 6px);`,
                            n.setAttribute("viewBox", "0 0 512 240");
                            const o = ((t < 29 ? e.rating + s : c) / e.difficulty) ** .5 * 45 + 55
                              , a = o < 70 ? 70 : o < 100 ? o : l < e.difficulty && e.acc < 100 ? 100 : o;
                            n.classList.add(a < 100 ? "sb" : l < e.difficulty && e.acc < 100 ? "sg" : "sr");
                            const d = e.songsName
                              , f = `#${t < 3 ? "φ" + t : t - 3}`;
                            if (d)
...
function getByToken(){
    MyUtil.createPopupBox('查询中，请耐心等待……', close=1);
    ...
    async function main(){
        ...
        try{
            var out = await pyodide.runPythonAsync(`
                ...
                [method.summary(), method.b19()]
            `);
        }catch(e){
            e = e + "";
            console.log(e)
            if(e.includes('IndexError')){
                alert('Token错误，请检查后重试。')
            }else if(e.includes('KeyError')){
                alert('数据库未更新，请联系作者更新。')
            }
            return;
        }
        out = out.toJs();
        ...
        var data = summary.get('money');
        function formatMoney(lst) {
            let str = "";
            for(let i = 4; i >= 0; i--){
                if(lst[i] != 0){
                    str += lst[i] + ["KB", "MB", "GB", "TB", "PB"][i] + " ";
                }
            }
            return str.trim();
        }
        var rks = 0;
        var o = b19;
        var a = o.slice(0, 30).reduce(((e,t)=>e + 5 * t.get('rks')), 0)
        , s = ((a + .5 | 0) - a + .5) / 5
        , l = o[2].get('rks')
        , c = o[29].get('rks') + s;
        ...
        const i = e=>n.measureText(e).width;
        d.innerHTML += `Player ID: ${summary.get('player_id')}`;
        d.classList.add("bi");
        d.innerHTML += `\nData: ${formatMoney(data)}`;
        document.getElementById("container").append(d, document.createElement("br"));
        var challenge = summary.get('challengeModeRank');
        if(challenge){
            challenge = '' + challenge;
            ...
            o.forEach((e,t)=>{
                ...
                /*if(t == 0 && !b19[1]){
                    ...
                    if(t < 30) rks += e.get('rks');
                    const n = document.createElementNS("http://www.w3.org/2000/svg", "svg");
                    n.classList.add("svg"),
                    n.style.cssText += `;width:calc(${100 / r}% - 6px);`,
                    n.setAttribute("viewBox", "0 0 512 240");
                    const o = ((t < 29 ? e.get('rks') + s : c) / e.get('difficulty')) ** .5 * 45 + 55
                    , a = o < 70 ? 70 : o < 100 ? o : l < e.get('difficulty') && e.get('acc') < 100 ? 100 : o;
                    n.classList.add(a < 100 ? "sb" : l < e.get('difficulty') && e.get('acc') < 100 ? "sg" : "sr");
                    const d = e.get('songname'), f = `#${t < 3 ? "φ" + t : t - 3}`;
                    if (d)
            ...
            d.innerHTML+=`\nRankingScore: ${(rks / 30).toFixed(4)}`;
            d.innerHTML+='\n背景颜色说明: <span class="sb">继续推分</span> <span class="sg">需要收歌才能涨rks</span> <span class="sr">无法推分</span>'
            document.getElementById("config").style.display = "";
            let b19_checkbox = document.getElementById("b19");
            b19_checkbox.onclick = () => {
                let container = document.getElementById("container");
                if(b19_checkbox.checked){
                    container.childNodes.forEach((val, idx) => val.style.display = idx < 32 ? "" : "none");
                }else{
                    container.childNodes.forEach((val, idx) => val.style.display = "");
                }
            }
        }
    }
