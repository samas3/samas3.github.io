...
class Stat {
	...
	get scoreNum() {
		...
		const c = 1e6 * (this.perfect + this.good * 0.65) / this.numOfNotes;
		if(this.config.lesson) return (isFinite(c) ? c : 0) + this.perfectMax * (this.config.yanpan);
		return (hyperMode.checked ? (isFinite(b) ? b : 0) : (isFinite(a) ? a : 0)) + this.perfectMax * (this.config.yanpan);
	}
	get scoreStr() {
		var a = this.scoreNum.toFixed(0);
		switch(this.config.mode){
			case 'normal':
				break;
			case 'minus':
				a = 1e6 - this.scoreNum.toFixed(0);
				break;
			case 'lagrange':
				var maxp = this.numOfNotes - eval(this.noteRank.join('+'));
				return this.toLagrange(this.score(this.perfect + maxp, this.good, this.combo + maxp, this.numOfNotes).toFixed(0));
		}
		return this.toScoreStr(a);
	}
	get scoreStrNum() {
		return this.toScoreStr(this.scoreNum.toFixed(0));
	}
	get accNum() {
	...
	get data_kb(){
		var max_data_kb = [0, 256, 256, 256, 256, 256, 256, 512, 512, 512, 768, 768, 768, 1024, 1024, 1280, 1536];
		if(this.scoreNum < 880000) return '0.00 KB';
		var kb = (this.scoreNum - 700000) / 300000 * max_data_kb[Math.floor(this.level)];
		var str = ' KB';
		if(kb >= 1024){
			kb /= 1024;
			str = ' MB';
		}
		if(!isFinite(kb)) return '0.00 KB';
		var f = Math.round(kb * 100) / 100;
		var s = f.toString();
		var rs = s.indexOf('.');
		if (rs < 0) {
			rs = s.length;
	...
	toScoreStr(s, num = 7){
		var a = '' + s;
		return ('0').repeat(a.length < num ? num - a.length : 0) + a;
	}
	toLagrange(a){
		var s;
		if(a < 7e5) s = 'C -' + this.toScoreStr(Math.min(7e5 - a, 99999) + this.perfectMax, 5);
		else if(a < 7.6e5) s = 'C +' + this.toScoreStr(a - 7e5 + this.perfectMax, 5);
		else if(a < 8.2e5) s = 'B -' + this.toScoreStr(8.2e5 - a + this.perfectMax, 5);
		else if(a < 8.5e5) s = 'B +' + this.toScoreStr(a - 8.2e5 + this.perfectMax, 5);
		else if(a < 8.8e5) s = 'A -' + this.toScoreStr(8.8e5 - a + this.perfectMax, 5);
		else if(a < 9e5) s = 'A +' + this.toScoreStr(a - 8.8e5 + this.perfectMax, 5);
		else if(a < 9.2e5) s = 'S -' + this.toScoreStr(9.2e5 - a + this.perfectMax, 5);
		else if(a < 9.4e5) s = 'S +' + this.toScoreStr(a - 9.2e5 + this.perfectMax, 5);
		else if(a < 9.6e5) s = 'V -' + this.toScoreStr(9.6e5 - a + this.perfectMax, 5);
		else if(a < 9.8e5) s = 'V +' + this.toScoreStr(a - 9.6e5 + this.perfectMax, 5);
		else if(a < 1e6) s = 'φ -' + this.toScoreStr(1e6 - a + this.perfectMax, 5);
		else s = 'φ +' + this.toScoreStr(Math.min(a - 1e6, 99999) + this.perfectMax, 5);
		return s;
	}
	score(p, g, m, n){
		const a = 1e6 * (p * 0.9 + g * 0.585 + m * 0.1) / n;
		return (isFinite(a) ? a : 0);
	}

	getData(isAuto, speed = '', isRandom, isEarly) {
		...
		const pbj = {
			...
			textAboveStr: !this.config.change_text ? '  ( {SPEED} 倍速 )' : '',

			textBelowColor: '#fe4365',
			textBelowStr: 'AUTO PLAY',
		};
		if(isEarly && !this.config.change_text) Object.assign(pbj, {textBelowStr:'EARLY PLAY', textBelowColor: '#00bef1'})
		if(isRandom && !this.config.change_text) Object.assign(pbj, {textBelowStr:'RANDOM PLAY', textBelowColor: '#19198a'})
		
		if (isAuto && !this.config.change_text) return Object.assign(pbj, { newBestColor: '#fff', newBestStr: 'BEST', scoreDelta: '' });
	...
	addCombo(status, type) {
		...
		if(status % 4 == 1 || status == 4){
			if(status == 4) this.perfectMax++;
			p.value = (this.noteRank[4] + this.noteRank[1] + this.noteRank[5]) + '(' + this.perfectMax + ')';
		}
		$("good").value = this.noteRank[3] + this.noteRank[7];
		$("bad").value = this.noteRank[6];
		$("miss").value = this.noteRank[2];
		if(status % 4 == 2) this.health -= 5;
		else if(status % 4 == 3) this.health -= 2;
		else if(status % 4 == 1) this.health -= 1;
		else{
			this.health += 1;
