...
function adjustInfo() {
	for (const i of chartInfoData) {
		if (selectchart.value.trim() === i.Chart) {
			...
			if (i.Level) {
				const p = i.Level.split('Lv.');
				if (p[0]) inputLevel.value = p[0];
				for(var j = 2; j < p.length; j++){
					p[1] += 'Lv.' + p[j];
				}
				if (p[1]) numLevel.value = p[1];
			}
...
function getJudgeDistance(judgeEvent, note) {
	if(randomplay.checked) return (Math.random()<0.5)^(Math.round(judgeEvent.offsetX*judgeEvent.offsetY/judgeEvent.type)%1919810>114514)&&(Math.random()<0.3);
...
const judgeManager = {
	...
	execute(notes, realTime, width) {
		...
		for (const note of notes) {
			...
			if(qwqmode.checked) stat.addCombo(4,2);
			if(randomplay.checked){
				note.status = Math.floor(Math.random() * 6 + 1);
				stat.addCombo(note.status, note.type);
				note.scored = true;
				qwq114514 = 1;
				//continue;
			}
			...
			if (deltaTime > 0.2 * judge) break; //跳过判定范围外的Note
			if (note.type !== 1 && deltaTime > 0.16 * judge) continue;
			if ((deltaTime < (hyperMode.checked ? -0.12 : -0.16) * judge && note.frameCount > (hyperMode.checked ? 3 : 4)) && !note.holdStatus) { //超时且不为Hold拖判，判为Miss
				...
				if (note.status !== 4) {
					...
					if(app.playMode == 1) stat.addInfo((realTime * app.speed).toFixed(3) + ' click ' + (note.projectX / canvasos.width).toFixed(3) + ' ' + (note.projectY / canvasos.height).toFixed(3));
					stat.addCombo(4, 2);
					note.scored = true;
				}
				...
				if (note.status !== 4) {
					for (const judgeEvent of list) {
						...
						for (const nearNote of note.nearNotes) {
							...
							if (nearNote.realTime - realTime > 0.16 * judge) break;
					...
					hitImageList.add(HitImage.perfect(note.projectX, note.projectY, note));
					if(app.playMode == 1) stat.addInfo((realTime * app.speed).toFixed(3) + ' move ' + (note.projectX / canvasos.width).toFixed(3) + ' ' + (note.projectY / canvasos.height).toFixed(3));
					stat.addCombo(4, 4);
					note.scored = true;
				}
			} else { //Hold音符
				if (note.type === 3 && note.holdTapTime) { //是否触发头判
					if ((performance.now() - note.holdTapTime) * note.holdTime >= 1.6e4 * note.realHoldTime * judge) { //间隔时间与bpm成反比
						if (note.holdStatus % 4 === 0) hitImageList.add(HitImage.perfect(note.projectX, note.projectY, note));
						else if (note.holdStatus % 4 === 1) hitImageList.add(hyperMode.checked ? HitImage.great(note.projectX, note.projectY, note) : HitImage.perfect(note.projectX, note.projectY, note));
					...
					if (deltaTime + note.realHoldTime < 0.2 * judge) {
						if (!note.status) stat.addCombo(note.status = note.holdStatus, 3);
						if (deltaTime + note.realHoldTime < 0) note.scored = true;
						if(app.playMode == 1) stat.addInfo((realTime * app.speed).toFixed(3) + ' up ' + (note.projectX / canvasos.width).toFixed(3) + ' ' + (note.projectY / canvasos.height).toFixed(3));
						continue;
					}
				...
				for (const judgeEvent of list) {
					...
					for (const nearNote of note.nearNotes) {
						...
						if (nearDeltaTime > 0.2 * judge) break;
						if (nearNote.type === 3 && nearDeltaTime > 0.16 * judge) continue;
						if (getJudgeOffset(judgeEvent, nearNote) > width) continue;
						const nearDistance = getJudgeDistance(judgeEvent, nearNote);
						if (nearDistance < distance) {
					...
					if(earlyplay.checked){
						deltaTime2 = 0.09;
					}

					if (deltaTime2 > (hyperMode.checked ? 0.12 : 0.16) * judge) {
						...
						stat.addDisp(Math.max(deltaTime2, (-1 - note.frameCount) * (hyperMode.checked ? 0.03 : 0.04) * judge || 0));
						if(note.type == 1) audio.play(res['HitSong0'], { gainrate: app.soundVolume });
						else audio.play(res['HitSong3'], { gainrate: app.soundVolume });
						if (deltaTime2 > (hyperMode.checked ? 0.06 : 0.08) * judge) {
							note.holdStatus = 7; //console.log('Good(Early)', i.name);
							hitImageList.add(HitImage.good(note.projectX, note.projectY, note));
							hitWordList.add(HitWord.early(note.projectX, note.projectY, deltaTime2));
						} else if (deltaTime2 > (hyperMode.checked ? 0.03 : 0.04) * judge) {
							if(lesson.checked){
								note.holdStatus = 7; //console.log('Good(Early)', i.name);
								hitImageList.add(HitImage.good(note.projectX, note.projectY, note));
								hitWordList.add(HitWord.early(note.projectX, note.projectY, deltaTime2));
						...
						} else if (deltaTime2 > -(hyperMode.checked ? 0.03 : 0.04) * judge || note.frameCount < 1) {
							noteJudge.holdStatus = 4; //console.log('Perfect(Max)', i.name);
							hitImageList.add(HitImage.perfect(note.projectX, note.projectY, note));
							if(note.type == 1 && app.playMode == 1) stat.addInfo((realTime * app.speed).toFixed(3) + ' click ' + (note.projectX / canvasos.width).toFixed(3) + ' ' + (note.projectY / canvasos.height).toFixed(3));
							else if(note.type == 3 && app.playMode == 1) stat.addInfo((realTime * app.speed).toFixed(3) + ' down ' + (note.projectX / canvasos.width).toFixed(3) + ' ' + (note.projectY / canvasos.height).toFixed(3));
						} else if (deltaTime2 > -(hyperMode.checked ? 0.06 : 0.08) * judge || note.frameCount < 2) {
							if(lesson.checked){
								note.holdStatus = 3; //console.log('Good(Late)', i.name);
								hitImageList.add(HitImage.good(note.projectX, note.projectY, note));
								hitWordList.add(HitWord.late(note.projectX, note.projectY, deltaTime2));
							}else{
								note.holdStatus = 1; //console.log('Perfect(Late)', i.name);
								hitImageList.add(hyperMode.checked ? HitImage.great(note.projectX, note.projectY, note) : HitImage.perfect(note.projectX, note.projectY, note));
								hitWordList.add(HitWord.late(note.projectX, note.projectY, deltaTime2));
							}
						} else {
							note.holdStatus = 3; //console.log('Good(Late)', i.name);
...
class HitImage {
	...
	static great(offsetX, offsetY) {
		return new HitImage(offsetX, offsetY, 'rgba(168,255,177,0.9016907)', 4, '#a8ffb1');
	}
	...
	static great(offsetX, offsetY, note) {
		// console.log(note);
		return new HitImage(offsetX, offsetY, 'Great', '#a8ffb1');
	}
...
class HitWord {
	...
	static early(offsetX, offsetY, delta) {
		//console.log('Tap', offsetX, offsetY);
		return new HitWord(offsetX, offsetY, '#03aaf9', 'Early' + (Number.isFinite(delta)?' +'+Math.abs(Math.round(delta*1000)):''));
	}
	static late(offsetX, offsetY, delta) {
		//console.log('Hold', offsetX, offsetY);
		return new HitWord(offsetX, offsetY, '#ff4612', 'Late' + (Number.isFinite(delta)?' -'+Math.abs(Math.round(delta*1000)):''));
	}
}
...
window.addEventListener('load', async function() {
	...
	await Promise.all(Object.entries({
		...
		HitSong3: 'src/HitSong3.ogg',
		LevelOver0_v1: 'src/LevelOver0_v1.ogg',
		LevelOver1_v1: 'src/LevelOver1_v1.ogg',
		LevelOver2_v1: 'src/LevelOver2_v1.ogg',
		LevelOver3_v1: 'src/LevelOver3_v1.ogg',
		...
		xhr.onloadend = async () => {
			...
			} else if (/\.(mp3|wav|ogg)$/i.test(src)) res[name] = await audio.decode(xhr.response); // 本地文件
			else if (/\.(png|jpeg|jpg)$/i.test(src)) res[name] = await createImageBitmap(new Blob([xhr.response])); // 本地文件
			else {
	...
	for (const i of entries){
		if(i.indexOf('dHL') == -1) await noteRender.update(i, res[i], 1);
		else await noteRender.update(i, res[i], 8080 / 7875);
	}
	await noteRender.updateFX(res['HitFXRaw'], 1);
	...
	res['JudgeLineAP'] = await imgShader(res['JudgeLine'], '#a3ffac');
...
function mainLoop() {
	...
	if (qwqOut.second < 0.67) {
		...
		if(cnt % 2) frameAnimater.stop();
		else{
			if(app.playMode == 1){
				$id('get-taxy').hidden = false;
				$id('get-taxy').onclick = function(){
					var numlvl = numLevel.value || numLevel.placeholder;
					if(numlvl.indexOf('.') > -1){
						numlvl = numlvl.substring(0, numlvl.indexOf('.'));
					}
					saveAs(new File([(inputName.value || inputName.placeholder) + '\n' + (inputLevel.value || inputLevel.placeholder) + ' Lv.' + numlvl + '\n' + stat.taxy], 'TAXY', { type: "charset=utf-8" }));
				}
			}
			fucktemp1 = true;
			qwqPause();
			...
			setTimeout(() => {
				...
				qwqEnd.reset();
				qwqEnd.play();
				stat.level = Number(numLevel.value);
				fucktemp2 = stat.getData(app.playMode === 1, selectspeed.value, randomplay.checked, earlyplay.checked);
			}, 1e3);
		}
	} //只让它执行一次
	if(qwq114514 && !fucktemp1){
		fucktemp1 = true;
		qwqPause();
		audio.stop();
		btnPause.classList.add('disabled'); //qwq
	...
	//Copyright
	ctx.font = `${lineScale * 0.4}px ${fonts.choice},Noto Sans SC`;
	ctx.fillStyle = '#ccc';
	ctx.globalAlpha = 0.8;
	ctx.textAlign = 'right';
	ctx.fillText(copyrightStr, (canvas.width + canvasos.width) / 2 - lineScale * 0.1, canvas.height - lineScale * 0.2);
	if(emitter.eq('play')) $id('duration').value = Math.round(main.time);
	// if(stat.health <= 0 && hyperMode.checked) qwq114514 = 1;
}
...
function loopNoCanvas() {
	...
	if (timeBgm >= duration || qwq114514) isOutStart = true;
...
function loopCanvas() { //尽量不要在这里出现app
	...
	if (qwqIn.second >= 3 && qwqOut.second === 0) {
		...
		if (showPoint.checked) { //绘制定位点
			ctxos.font = `${lineScale}px ${fonts.choice},Noto Sans SC`;
			ctxos.textAlign = 'center';
	...
	ctxos.textAlign = 'center';
	ctxos.font = `${lineScale * 1.32}px ${fonts.choice},Noto Sans SC`;
	if(stat.combo > 2 || show_combo_all.checked || tmps.combo.indexOf('(') > -1){
		if(!show_combo_all.checked) ctxos.fillText(tmps.combo, app.wlen, lineScale * 1.375);
		else ctxos.fillText(tmps.combo + '/' + eval(stat.noteRank.join('+')), app.wlen, lineScale * 1.375);
		ctxos.globalAlpha = qwqIn.second < 0.67 ? tween.easeOutSine(qwqIn.second * 1.5) : (1 - tween.easeOutSine(qwqOut.second * 1.5));
		ctxos.font = `${lineScale * 0.5}px ${fonts.choice},Noto Sans SC`;
		ctxos.fillText(earlyplay.checked && !change_text.checked ? 'EARLYPLAY' : (randomplay.checked && !change_text.checked ? 'RANDOMPLAY' : (app.playMode === 1 && !change_text.checked ? typeStr : 'COMBO')), app.wlen, lineScale * 2.05);
	}
	if(0 && hyperMode.checked){
		ctxos.font = `${lineScale * 0.65}px ${fonts.choice},Noto Sans SC`;
		ctxos.textAlign = 'center';
		ctxos.fillText('HyperMode Health ' + stat.health + '%', app.wlen, lineScale * 2.65);
	}
	//绘制曲名和等级
	ctxos.globalAlpha = 1;
	...
	ctxos.font = `${lineScale * 0.4}px ${fonts.choice},Noto Sans SC`;
	ctxos.textAlign = 'left';
...
function fillTextNode(text, x, y, size, maxWidth) {
	ctxos.font = `${size}px ${fonts.choice},Noto Sans SC`;
	const dx = ctxos.measureText(text).width;
	if (dx > maxWidth) ctxos.font = `${size / dx * maxWidth}px ${fonts.choice},Noto Sans SC`;
...
function qwqdraw3(statData) {
	...
	if(ui.choice == '1'){
		...
		ctxos.font = `30px ${fonts.choice},Noto Sans SC`;
		//Rank图标
		...
		if(lesson.checked) ctxos.fillText('未使用', 1528, 550);
		else ctxos.fillText(stat.maxcombo.toString(), 1528, 550);

		// Custom Data
		if(!original.checked){
			ctxos.fillText('单曲RKS: ' + stat.rks, 220, 580);
			ctxos.fillText('Data: ' + stat.data_kb, 1380, 515);
		}
		if(hyperMode.checked){
			if(stat.health <= 0) ctxos.fillText('HyperMode Failed', 200, 645);
			else ctxos.fillText('HyperMode ' + stat.health + '%', 200, 645);
		}
		// End

		ctxos.fillStyle = statData.textAboveColor;
		ctxos.fillText(app.speed === 1 ? '' : statData.textAboveStr.replace('{SPEED}', app.speed), 383 + Math.min(lw, 750), 212);
		ctxos.fillStyle = statData.textBelowColor;
		ctxos.fillText(statData.textBelowStr, 1355, 595);
		ctxos.fillStyle = '#fff';
		ctxos.textAlign = 'center';
		ctxos.font = `86px ${fonts.choice},Noto Sans SC`;
		ctxos.globalAlpha = clip((qwqEnd.second - 1.12) * 2.00);
		ctxos.fillText(stat.scoreStrNum, 1075, 569);
		ctxos.font = `26px ${fonts.choice},Noto Sans SC`;
		ctxos.globalAlpha = clip((qwqEnd.second - 0.87) * 2.50);

		if(yanpan.checked) ctxos.fillText(stat.perfect + '(' + stat.perfectMax + ')', 891, 650);
		else ctxos.fillText(stat.perfect.toString(), 891, 650);
		...
		ctxos.font = `22px ${fonts.choice},Noto Sans SC`;
...
const noteRender = {
	...
	async updateFX(img, scale, limitX, limitY, hideParts, duration) {
		...
		const hitGreat = hitRaw.map(async img => new ScaledNote(await imgShader(img, 'rgba(168,255,177,0.9016907)'), scale)); //#a8ffb1
		...
		this.hitFX['Great'] = {
			effects: await Promise.all(hitGreat),
			numOfParts: hideParts ? 0 : 4,
			duration: duration | 0 || 500
		};
		this.hitFX['Good'] = {
...
class Choice {
	constructor(text, choices, values, def = 0){
		this.container = document.createElement('div');
		this.container.innerHTML = text;
		this.select = document.createElement('select');
		this.select.id = Utils.randomUUID();
		for(var i = 0; i < choices.length; i++){
			this.select.options.add(new Option(choices[i], values[i], (def == i)));
		}
...
const lesson = new Checkbox('课题模式').appendTo($id('view-cfg')).hook(status.reg.bind(status, 'lesson'));
const change_text = new Checkbox('界面不改字').appendTo($id('view-cfg')).hook(status.reg.bind(status, 'change_text'));
const show_combo_all = new Checkbox('combo附加功能').appendTo($id('view-cfg')).hook(status.reg.bind(status, 'show_combo_all'));
const original = new Checkbox('纯净模式').appendTo($id('view-cfg')).hook(status.reg.bind(status, 'original'));
const dev = new Checkbox('调试模式').appendTo($id('view-cfg')).hook(status.reg.bind(status, 'dev'));
yanpan.checkbox.addEventListener('change', () => {
	stat.setConfig('yanpan', yanpan.checked);
});
lesson.checkbox.addEventListener('change', () => {
	stat.setConfig('lesson', lesson.checked);
});
change_text.checkbox.addEventListener('change', () => {
	stat.setConfig('change', change_text.checked);
});
dev.checkbox.addEventListener('change', evt => {
	const tags = ["perfect", "good", "bad", "miss", "addp", "addg", "addb", "addm", "end"];
	for(var i of tags){
		$id(i).hidden = !evt.target.checked;
	}
});
...
$id('record').addEventListener('click', () => {
	...
	$id('showTransition').checked = !$id('showTransition').checked;
	original.checkbox.checked = !original.checkbox.checked;
	cnt++;
	typeStr = strs[cnt % 2];
...
selectspeed.addEventListener('change', ( /** @type {Event&{target:HTMLInputElement}} */ evt) => {
	app.speed = Number(evt.target.value);
});
const judgerate = $id('judge');
var judge = 1;
judgerate.addEventListener('change', evt => {
	judge = Number(judgerate.value);
});
...
const inputLevel = $id('input-level');
const numLevel = $id('numlevel');
$id('del_chart').addEventListener('click', () => {
	var sel = selectchart;
...
$id('del_bgm').addEventListener('click', () => {
	var sel = selectbgm;
	var index = sel.selectedIndex;
	if(index > -1){
		sel.remove(index);
...
$id('del_bg').addEventListener('click', () => {
	var sel = selectbg;
	var index = sel.selectedIndex;
	if(index > -1){
		sel.remove(index);
...
$id('jump').addEventListener('click', () => {
	main.time = Number($id('duration').value);
});
$id('random').addEventListener('click', () => {
	var sel = selectchart;
	var l = sel.length;
	var index = Math.floor(Math.random() * l);
	sel[index].selected = 1;
...
status2.reg(emitter, 'change', _ => main.qwqwq ? '翻转' : ''); //qwq
status2.reg(selectflip, 'change', target => ['', '翻转X', '翻转Y', '翻转XY'][target.value]);
status2.reg(selectspeed, 'change', target => target.value + '倍速');
status2.reg(emitter, 'change', ( /** @type {Emitter} */ target) => target.eq('pause') ? '暂停' : '');
async function qwqStop() {
	if (emitter.eq('stop')) {
		...
		$id('get-taxy').hidden = true;
		stat.setConfig('yanpan', yanpan.checked);
		stat.setConfig('lesson', lesson.checked);
		stat.setConfig('change', change_text.checked);
		stat.setConfig('mode', mode.choice);
		levelText = (inputLevel.value || inputLevel.placeholder) + ' Lv.' + (numLevel.value || numLevel.placeholder);
...
async function loadLineData() {
	for (const i of app.lines) {
		...
		i.imageH = 7.68; //3
		i.imageL = [res['JudgeLine'], res['JudgeLineMP'], res['JudgeLineAP'], res['JudgeLineFC']];
		i.imageS = 1; //2.56
		i.imageA = 1; //1.5625
		i.imageD = false;
		i.imageC = true;
